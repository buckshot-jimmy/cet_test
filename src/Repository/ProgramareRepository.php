...
class ProgramareRepository extends ServiceEntityRepository
{
    const COL_PACIENT = "1";
    const COL_MEDIC = "2";
    ...
    {
        parent::__construct($registry, Programare::class);
    }
    ...
    public function saveProgramare($formData, $adaugataDe)
    {
        ...
        if (isset($formData['programare_id']) && !empty($formData['programare_id'])) {
            $programare = $this->em->getRepository(Programare::class)->find($formData['programare_id']);
        }

        if(!$programare instanceof Programare) {
            throw new BadRequestHttpException("Missing ID");
        }

        $pacient = $this->em->getRepository(Pacient::class)->find($formData['programare_pacient']);
        $pret = $this->em->getRepository(Pret::class)->find($formData['programare_pret_serviciu']);
    ...
    {
        $programare = $this->em->getRepository(Programare::class)->find($programareId);
